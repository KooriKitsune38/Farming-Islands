...
$execute unless entity @s[tag=.temp] store result score .alreadyCollaborator coop if data storage kfi:values coopIslands[{UUID:$(ownerUUID)}].players[{UUID:$(tempUUID)}]

execute if score .success k.Values matches 1 run tellraw @s[tag=!.temp] [{"text":"| ","color":"gray"},{"color":"red","text":"You're already a collaborator!"}]
